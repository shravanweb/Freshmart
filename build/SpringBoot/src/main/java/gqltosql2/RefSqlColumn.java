...
public class RefSqlColumn implements ISqlColumn {
	...
	public RefSqlColumn(SqlAstNode sub, String column, String field, boolean absolute) {
		this.sub = sub;
		this.column = column;
		this.field = field;
	...
	public void addColumn(SqlTable table, SqlQueryContext ctx) {
		if (sub.isEmbedded()) {
			...
			String idColumn = absolute ? column : (ctx.getFrom() + '.' + column);
			QueryReader reader = ctx.addRefSelection(idColumn, field);
			SqlQueryContext prefix = ctx.subPrefix(getFieldName());
			String join = prefix.getTableAlias(String.valueOf(sub.getType()));
			ctx.addJoin(sub.getTableName(), join, join + "._id = " + idColumn);
			SqlQueryContext sc = prefix.subReader(reader);
			sc.addSqlColumns(sub);
		}
	}
