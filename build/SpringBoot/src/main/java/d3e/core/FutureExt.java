...
public class FutureExt<T> {
	...
	public static <T> Single<T> Future(Supplier<T> computation) {
		if (computation == null) {
			throw new RuntimeException("computation is required");
		}
		T t = computation.get();
	...
	public static <R, T> Single<R> orElse(Single<T> on, Function<T, R> onValue,
			@Param("onError") Function<T, Single<R>> onElse) {
		return on.concatMap(t -> {
			R v = onValue.apply(t);
			if(v == null) {
				return onElse.apply(t);
			} else {
				return value(v);
			}
		});
	...
	public static <R, T> Single<R> then(Single<T> on, Function<T, R> onValue,
			@Param("onError") Function<T, R> onError) {
		return on.map(v -> onValue.apply(v));
	}
