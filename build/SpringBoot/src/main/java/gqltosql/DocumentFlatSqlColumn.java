...
public class DocumentFlatSqlColumn implements ISqlColumn {
	...
	@Override
	public void extractDeepFields(IEntityManager em, IModelSchema schema, String type, List<SqlRow> rows)
			throws Exception {
			...
			try {
				...
				for (String fn : flatPaths) {
					if (o.has(fn)) {
						...
						DatabaseObject obj = JSONInputContext.fromJsonString(doc, o.getLong("id"), df.getReference().getType(), schema);
