...
public class AllStockAlerts extends DBObject {
  ...
  private long count;
  private List<StockAlert> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllStockAlerts() {}

  public AllStockAlerts(
      long count, List<String> errors, List<StockAlert> items, ResultStatus status) {
    this.count = count;
  ...
  public List<StockAlert> getItems() {
    return items;
  }
  ...
  public void setItems(List<StockAlert> items) {
    ((D3EPersistanceList<StockAlert>) this.items).setAll(items);
  }
  ...
  public void addToItems(StockAlert val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(StockAlert val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllStockAlerts;
  }
