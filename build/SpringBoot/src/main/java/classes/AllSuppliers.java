...
public class AllSuppliers extends DBObject {
  ...
  private long count;
  private List<Vendor> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllSuppliers() {}

  public AllSuppliers(long count, List<String> errors, List<Vendor> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<Vendor> items) {
    ((D3EPersistanceList<Vendor>) this.items).setAll(items);
  }
  ...
  public void addToItems(Vendor val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(Vendor val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllSuppliers;
  }
