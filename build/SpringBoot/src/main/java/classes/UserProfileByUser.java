...
public class UserProfileByUser extends DBObject {
  ...
  private List<UserProfile> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public UserProfileByUser() {}

  public UserProfileByUser(List<String> errors, List<UserProfile> items, ResultStatus status) {
  ...
  public void setItems(List<UserProfile> items) {
    ((D3EPersistanceList<UserProfile>) this.items).setAll(items);
  }
  ...
  public void addToItems(UserProfile val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(UserProfile val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.UserProfileByUser;
  }
