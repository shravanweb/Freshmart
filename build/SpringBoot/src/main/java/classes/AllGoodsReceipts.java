...
public class AllGoodsReceipts extends DBObject {
  ...
  private long count;
  private List<GoodsReceipt> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllGoodsReceipts() {}

  public AllGoodsReceipts(
      long count, List<String> errors, List<GoodsReceipt> items, ResultStatus status) {
    this.count = count;
  ...
  public List<GoodsReceipt> getItems() {
    return items;
  }
  ...
  public void setItems(List<GoodsReceipt> items) {
    ((D3EPersistanceList<GoodsReceipt>) this.items).setAll(items);
  }
  ...
  public void addToItems(GoodsReceipt val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(GoodsReceipt val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllGoodsReceipts;
  }
