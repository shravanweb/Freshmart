...
public class AllSalesReturns extends DBObject {
  ...
  private long count;
  private List<SalesReturn> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllSalesReturns() {}

  public AllSalesReturns(
      long count, List<String> errors, List<SalesReturn> items, ResultStatus status) {
    this.count = count;
  ...
  public List<SalesReturn> getItems() {
    return items;
  }
  ...
  public void setItems(List<SalesReturn> items) {
    ((D3EPersistanceList<SalesReturn>) this.items).setAll(items);
  }
  ...
  public void addToItems(SalesReturn val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(SalesReturn val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllSalesReturns;
  }
