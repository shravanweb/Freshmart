...
public class AllStores extends DBObject {
  ...
  private long count;
  private List<Store> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllStores() {}

  public AllStores(long count, List<String> errors, List<Store> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<Store> items) {
    ((D3EPersistanceList<Store>) this.items).setAll(items);
  }
  ...
  public void addToItems(Store val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(Store val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllStores;
  }
