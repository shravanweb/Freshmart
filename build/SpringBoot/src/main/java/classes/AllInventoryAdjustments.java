...
public class AllInventoryAdjustments extends DBObject {
  ...
  private long count;
  private List<InventoryAdjustment> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllInventoryAdjustments() {}

  public AllInventoryAdjustments(
      long count, List<String> errors, List<InventoryAdjustment> items, ResultStatus status) {
    this.count = count;
  ...
  public List<InventoryAdjustment> getItems() {
    return items;
  }
  ...
  public void setItems(List<InventoryAdjustment> items) {
    ((D3EPersistanceList<InventoryAdjustment>) this.items).setAll(items);
  }
  ...
  public void addToItems(InventoryAdjustment val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(InventoryAdjustment val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllInventoryAdjustments;
  }
