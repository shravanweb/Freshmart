...
public class AllProductCategories extends DBObject {
  ...
  private long count;
  private List<ProductCategory> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllProductCategories() {}

  public AllProductCategories(
      long count, List<String> errors, List<ProductCategory> items, ResultStatus status) {
    this.count = count;
  ...
  public List<ProductCategory> getItems() {
    return items;
  }
  ...
  public void setItems(List<ProductCategory> items) {
    ((D3EPersistanceList<ProductCategory>) this.items).setAll(items);
  }
  ...
  public void addToItems(ProductCategory val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(ProductCategory val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllProductCategories;
  }
