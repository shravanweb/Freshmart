...
public class AllUnitOfMeasures extends DBObject {
  ...
  private long count;
  private List<UnitOfMeasure> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllUnitOfMeasures() {}

  public AllUnitOfMeasures(
      long count, List<String> errors, List<UnitOfMeasure> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<UnitOfMeasure> items) {
    ((D3EPersistanceList<UnitOfMeasure>) this.items).setAll(items);
  }
  ...
  public void addToItems(UnitOfMeasure val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(UnitOfMeasure val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllUnitOfMeasures;
  }
