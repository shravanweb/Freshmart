...
public class ProductSearch extends DBObject {
  ...
  private long count;
  private List<Product> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public ProductSearch() {}

  public ProductSearch(long count, List<String> errors, List<Product> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<Product> items) {
    ((D3EPersistanceList<Product>) this.items).setAll(items);
  }
  ...
  public void addToItems(Product val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(Product val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.ProductSearch;
  }
