...
public class WarehouseStockByWarehouse extends DBObject {
  ...
  private long count;
  private List<WarehouseStock> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public WarehouseStockByWarehouse() {}

  public WarehouseStockByWarehouse(
      long count, List<String> errors, List<WarehouseStock> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<WarehouseStock> items) {
    ((D3EPersistanceList<WarehouseStock>) this.items).setAll(items);
  }
  ...
  public void addToItems(WarehouseStock val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(WarehouseStock val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.WarehouseStockByWarehouse;
  }
