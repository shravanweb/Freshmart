...
public class AllInAppNotifications extends DBObject {
  ...
  private long count;
  private List<InAppNotification> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllInAppNotifications() {}

  public AllInAppNotifications(
      long count, List<String> errors, List<InAppNotification> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<InAppNotification> items) {
    ((D3EPersistanceList<InAppNotification>) this.items).setAll(items);
  }
  ...
  public void addToItems(InAppNotification val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(InAppNotification val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllInAppNotifications;
  }
