...
public class PurchaseOrdersByStatus extends DBObject {
  ...
  private long count;
  private List<PurchaseOrder> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public PurchaseOrdersByStatus() {}

  public PurchaseOrdersByStatus(
      long count, List<String> errors, List<PurchaseOrder> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<PurchaseOrder> items) {
    ((D3EPersistanceList<PurchaseOrder>) this.items).setAll(items);
  }
  ...
  public void addToItems(PurchaseOrder val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(PurchaseOrder val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.PurchaseOrdersByStatus;
  }
