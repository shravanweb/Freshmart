...
public class WarehousesByStore extends DBObject {
  ...
  private long count;
  private List<Warehouse> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public WarehousesByStore() {}

  public WarehousesByStore(
      long count, List<String> errors, List<Warehouse> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<Warehouse> items) {
    ((D3EPersistanceList<Warehouse>) this.items).setAll(items);
  }
  ...
  public void addToItems(Warehouse val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(Warehouse val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.WarehousesByStore;
  }
