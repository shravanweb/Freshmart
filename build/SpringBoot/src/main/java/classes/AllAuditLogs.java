...
public class AllAuditLogs extends DBObject {
  ...
  private long count;
  private List<AuditLog> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllAuditLogs() {}

  public AllAuditLogs(long count, List<String> errors, List<AuditLog> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<AuditLog> items) {
    ((D3EPersistanceList<AuditLog>) this.items).setAll(items);
  }
  ...
  public void addToItems(AuditLog val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(AuditLog val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllAuditLogs;
  }
