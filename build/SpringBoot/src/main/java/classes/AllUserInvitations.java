...
public class AllUserInvitations extends DBObject {
  ...
  private long count;
  private List<UserInvitation> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllUserInvitations() {}

  public AllUserInvitations(
      long count, List<String> errors, List<UserInvitation> items, ResultStatus status) {
    this.count = count;
  ...
  public List<UserInvitation> getItems() {
    return items;
  }
  ...
  public void setItems(List<UserInvitation> items) {
    ((D3EPersistanceList<UserInvitation>) this.items).setAll(items);
  }
  ...
  public void addToItems(UserInvitation val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(UserInvitation val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllUserInvitations;
  }
