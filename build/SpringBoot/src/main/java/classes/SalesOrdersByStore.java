...
public class SalesOrdersByStore extends DBObject {
  ...
  private long count;
  private List<SalesOrder> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public SalesOrdersByStore() {}

  public SalesOrdersByStore(
      long count, List<String> errors, List<SalesOrder> items, ResultStatus status) {
    this.count = count;
  ...
  public void setItems(List<SalesOrder> items) {
    ((D3EPersistanceList<SalesOrder>) this.items).setAll(items);
  }
  ...
  public void addToItems(SalesOrder val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(SalesOrder val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.SalesOrdersByStore;
  }
