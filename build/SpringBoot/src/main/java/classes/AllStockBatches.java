...
public class AllStockBatches extends DBObject {
  ...
  private long count;
  private List<StockBatch> items = new D3EPersistanceList<>(this, _ITEMS);
  private List<TypeAndId> itemsRef;

  public AllStockBatches() {}

  public AllStockBatches(
      long count, List<String> errors, List<StockBatch> items, ResultStatus status) {
    this.count = count;
  ...
  public List<StockBatch> getItems() {
    return items;
  }
  ...
  public void setItems(List<StockBatch> items) {
    ((D3EPersistanceList<StockBatch>) this.items).setAll(items);
  }
  ...
  public void addToItems(StockBatch val, long index) {
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(StockBatch val) {
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.AllStockBatches;
  }
