		...
		while (!shutdown) {
			try {
				...
				case Fire: {
					...
					doFire(toSend, event.obj2, event.changeType, event.changes);
					toSend.send(socket);
					emProvider.clear();
					break;
				}
				case Fire2: {
					...
					events.forEach(ev -> {
						if (ev.getEntity() instanceof DBObject) {
							doFire(toSend, (DBObject) ev.getEntity(), ev.getType(), event.changes);
						}
					});
					toSend.send(socket);
					emProvider.clear();
...
//		 + LISTEXT.MAP(SEL.GETFIELDS(), (F) -> F.GETField().getName()) + " Object Type: " + objType.getType() + " Type : " + type);
	...
	private void doFire(ObjectsToSend toSend, DBObject object, StoreEventType changeType, Map<DBObject, DBChange> changes) {
