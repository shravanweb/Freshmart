...
public abstract class AbstractChannels {
  ...
  public boolean connect(DClazz dm, ClientSession ses, EntityHelperService helperService) {
	  Log.info("Channel connected: " + dm.getName());
    ServerChannel channel = allChannels.get(dm.getName());
    AbstractClientProxy proxy = getChannelClientProxy(dm.getIndex(), ses, helperService, ses.template);
    if(proxy == null) {
    	Log.info("Unable to create Client Proxy for '" + dm.getName() + "' idx: " + dm.getIndex());
    	return false;
    }
  ...
  public void disconnect(DClazz dm, ClientSession ses) {
	Log.info("Channel disconnected: " + dm.getName());
    ServerChannel channel = allChannels.get(dm.getName());
  ...
  public void onMessage(DClazz dm, int msgSrvIdx, ClientSession sesssion, RocketInputContext ctx, EntityHelperService helperService) {
    ServerChannel channel = allChannels.get(dm.getName());
    if(!sesssion.proxies.containsKey(dm.getName())) {
