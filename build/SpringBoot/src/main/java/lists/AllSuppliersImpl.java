...
@Service
public class AllSuppliersImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllSuppliersRequest inputToRequest(AllSuppliersIn inputs) {
    AllSuppliersRequest request = new AllSuppliersRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllSuppliers get(AllSuppliersIn inputs) {
    AllSuppliersRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllSuppliers get(AllSuppliersRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllSuppliers getAsStruct(List<NativeObj> rows, long count) {
    List<Vendor> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.Vendor));
    }
    AllSuppliers wrap = new AllSuppliers();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllSuppliersIn inputs) throws Exception {
    AllSuppliersRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllSuppliersRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllSuppliersRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  }
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllSuppliers);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllSuppliersRequest request) {
    String sql =
        "select a._organization_id a0, a._name a1, a._id a2 from _vendor a where a._organization_id = :param_0 order by a._name";
  ...
  public long getCountResult(AllSuppliersRequest request) {
    String sql = "select count(a._id) a0 from _vendor a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
