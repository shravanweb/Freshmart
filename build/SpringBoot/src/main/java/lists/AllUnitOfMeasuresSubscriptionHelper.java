...
public class AllUnitOfMeasuresSubscriptionHelper
  ...
  private Field field;
  private AllUnitOfMeasuresRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allUnitOfMeasuresImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allUnitOfMeasuresImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllUnitOfMeasuresRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<UnitOfMeasure>>) subscription.onUnitOfMeasureChangeEvent())
            .subscribe((e) -> applyUnitOfMeasure(e));
    disposables.add(baseSubscribe);
  }

  public void applyUnitOfMeasure(D3ESubscriptionEvent<UnitOfMeasure> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    UnitOfMeasure model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      UnitOfMeasure old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, UnitOfMeasure model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createUnitOfMeasureData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, UnitOfMeasure model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, UnitOfMeasure model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, UnitOfMeasure model, UnitOfMeasure old) {
  ...
  private List<NativeObj> createUnitOfMeasureData(UnitOfMeasure unitOfMeasure) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, unitOfMeasure.getOrganization().getId());
    row.set(1, unitOfMeasure.getName());
    row.set(2, unitOfMeasure.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
