...
public class MovementReportRowsSubscriptionHelper
  ...
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        LocalDateTime OrderByValue0 = wrappedBase.getDateTime(1);
    ...
    public long getPath(LocalDateTime OrderByValue0) {
      long index = 0;
  ...
  private class OrderBy {
    ...
    public OrderBy(LocalDateTime OrderByValue0, Row row) {
  ...
  private Field field;
  private MovementReportRowsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = movementReportRowsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = movementReportRowsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, MovementReportRowsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<InventoryMovement>>)
                subscription.onInventoryMovementChangeEvent())
            .subscribe((e) -> applyInventoryMovement(e));
    disposables.add(baseSubscribe);
  }

  public void applyInventoryMovement(D3ESubscriptionEvent<InventoryMovement> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    InventoryMovement model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      InventoryMovement old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private boolean applyWhere(InventoryMovement model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization())
        && DateTimeExt.gte(model.getMovementDate(), inputs.getStartDate())
        && DateTimeExt.lte(model.getMovementDate(), inputs.getEndDate())
        && (inputs.getWarehouse() == null
            || Objects.equals(model.getWarehouse(), inputs.getWarehouse()));
  }

  private void createInsertChange(List<DataQueryDataChange> changes, InventoryMovement model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createInventoryMovementData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getMovementDate());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, InventoryMovement model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, InventoryMovement model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, InventoryMovement model, InventoryMovement old) {
    LocalDateTime oldValue0 = old.getMovementDate();
    LocalDateTime modelValue0 = model.getMovementDate();
    ...
    LocalDateTime OrderByValue0 = model.getMovementDate();
  ...
  private List<NativeObj> createInventoryMovementData(InventoryMovement inventoryMovement) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(4);
    row.set(0, inventoryMovement.getOrganization().getId());
    row.set(1, inventoryMovement.getMovementDate());
    row.set(2, inventoryMovement.getWarehouse().getId());
    row.set(3, inventoryMovement.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
