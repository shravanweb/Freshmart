...
@Service
public class AllProductsImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllProductsRequest inputToRequest(AllProductsIn inputs) {
  ...
  public AllProducts get(AllProductsIn inputs) {
    AllProductsRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllProducts get(AllProductsRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllProducts getAsStruct(List<NativeObj> rows, long count) {
    List<Product> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.Product));
    }
    AllProducts wrap = new AllProducts();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllProductsIn inputs) throws Exception {
    AllProductsRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllProductsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllProductsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  }
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllProducts);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllProductsRequest request) {
    String sql =
        "select a._organization_id a0, a._name a1, a._id a2 from _product a where a._organization_id = :param_0 order by a._name";
  ...
  public long getCountResult(AllProductsRequest request) {
    String sql = "select count(a._id) a0 from _product a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
