...
public class AllInventoryAdjustmentsChangeTracker implements Cancellable {
  ...
  private class AllInventoryAdjustmentsInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllInventoryAdjustmentsInput(AllInventoryAdjustmentsRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllInventoryAdjustments\\\" is required, but got value null.");
      }
    }
  ...
  private AllInventoryAdjustments root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllInventoryAdjustmentsInput inputs;
  private Field field;

  public AllInventoryAdjustmentsChangeTracker(
  ...
  public void init(
      OutObject out, AllInventoryAdjustments initialData, AllInventoryAdjustmentsRequest inputs) {
    {
    ...
    this.inputs = new AllInventoryAdjustmentsInput(inputs);
  ...
  private void storeInitialData(AllInventoryAdjustments initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getAdjustmentDate(), x.getId()))
            ...
            (obj, old, type) -> {
              try {
                ...
                applyInventoryAdjustment(
                    ((InventoryAdjustment) obj), ((InventoryAdjustment) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyInventoryAdjustment(
      InventoryAdjustment model, InventoryAdjustment old, StoreEventType type) {
  ...
  private void createInsertChange(InventoryAdjustment model) {
    long id = model.getId();
  ...
  private void createDeleteChange(InventoryAdjustment model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(InventoryAdjustment model, InventoryAdjustment old) {
  ...
  private void moveItems(InventoryAdjustment model, TypeAndId data, LocalDate OrderByValue0) {
    // Find old index
