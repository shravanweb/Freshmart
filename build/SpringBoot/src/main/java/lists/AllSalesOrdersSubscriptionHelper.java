...
public class AllSalesOrdersSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllSalesOrdersRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allSalesOrdersImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allSalesOrdersImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllSalesOrdersRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<SalesOrder>>) subscription.onSalesOrderChangeEvent())
            .subscribe((e) -> applySalesOrder(e));
    disposables.add(baseSubscribe);
  }

  public void applySalesOrder(D3ESubscriptionEvent<SalesOrder> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    SalesOrder model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, SalesOrder model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createSalesOrderData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getOrderDate());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, SalesOrder model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, SalesOrder model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, SalesOrder model, SalesOrder old) {
  ...
  private List<NativeObj> createSalesOrderData(SalesOrder salesOrder) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, salesOrder.getOrganization().getId());
    row.set(1, salesOrder.getOrderDate());
    row.set(2, salesOrder.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
