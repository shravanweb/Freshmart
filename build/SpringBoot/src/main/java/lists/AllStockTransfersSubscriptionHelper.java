...
public class AllStockTransfersSubscriptionHelper
  ...
  private Field field;
  private AllStockTransfersRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allStockTransfersImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allStockTransfersImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllStockTransfersRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<StockTransfer>>) subscription.onStockTransferChangeEvent())
            .subscribe((e) -> applyStockTransfer(e));
    disposables.add(baseSubscribe);
  }

  public void applyStockTransfer(D3ESubscriptionEvent<StockTransfer> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    StockTransfer model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      StockTransfer old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, StockTransfer model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createStockTransferData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getTransferDate());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, StockTransfer model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, StockTransfer model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, StockTransfer model, StockTransfer old) {
  ...
  private List<NativeObj> createStockTransferData(StockTransfer stockTransfer) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, stockTransfer.getOrganization().getId());
    row.set(1, stockTransfer.getTransferDate());
    row.set(2, stockTransfer.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
