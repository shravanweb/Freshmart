...
public class WarehouseStockByProductSubscriptionHelper
  ...
  private Field field;
  private WarehouseStockByProductRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = warehouseStockByProductImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = warehouseStockByProductImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(
      Field field, WarehouseStockByProductRequest inputs) {
    {
  ...
  private boolean applyWhere(WarehouseStock model) {
    return Objects.equals(model.getProduct(), inputs.getProduct());
  }
  ...
  private List<NativeObj> createWarehouseStockData(WarehouseStock warehouseStock) {
    ...
    row.set(0, warehouseStock.getProduct().getId());
