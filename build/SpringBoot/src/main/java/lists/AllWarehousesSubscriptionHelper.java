...
public class AllWarehousesSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllWarehousesRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allWarehousesImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allWarehousesImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllWarehousesRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<Warehouse>>) subscription.onWarehouseChangeEvent())
            .subscribe((e) -> applyWarehouse(e));
    disposables.add(baseSubscribe);
  }

  public void applyWarehouse(D3ESubscriptionEvent<Warehouse> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    Warehouse model = e.model;
  ...
  private boolean applyWhere(Warehouse model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, Warehouse model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createWarehouseData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, Warehouse model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, Warehouse model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, Warehouse model, Warehouse old) {
  ...
  private List<NativeObj> createWarehouseData(Warehouse warehouse) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, warehouse.getOrganization().getId());
    row.set(1, warehouse.getName());
    row.set(2, warehouse.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
