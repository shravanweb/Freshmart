...
public class WarehousesByStoreChangeTracker implements Cancellable {
  ...
  private class WarehousesByStoreInput {
    private TypeAndId storeRef;
    private Store store;

    public WarehousesByStoreInput(WarehousesByStoreRequest _req) {
      this.storeRef = TypeAndId.from(_req.getStore());
      if (_req.getStore() == null) {
        Log.info(
            "WARNING: Input \\\"store\\\" in \\\"WarehousesByStore\\\" is required, but got value null.");
      }
    }
  ...
  private WarehousesByStore root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private WarehousesByStoreInput inputs;
  private Field field;

  public WarehousesByStoreChangeTracker(
  ...
  public void init(OutObject out, WarehousesByStore initialData, WarehousesByStoreRequest inputs) {
    {
    ...
    this.inputs = new WarehousesByStoreInput(inputs);
  ...
  private void storeInitialData(WarehousesByStore initialData) {
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyWarehouse(((Warehouse) obj), ((Warehouse) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyWarehouse(Warehouse model, Warehouse old, StoreEventType type) {
  ...
  private void createInsertChange(Warehouse model) {
    long id = model.getId();
  ...
  private void createDeleteChange(Warehouse model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(Warehouse model, Warehouse old) {
  ...
  private void moveItems(Warehouse model, TypeAndId data, String OrderByValue0) {
    // Find old index
