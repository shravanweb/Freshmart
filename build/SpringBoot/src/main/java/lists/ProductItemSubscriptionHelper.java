...
public class ProductItemSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private ProductItemRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = productItemImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = productItemImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, ProductItemRequest inputs) {
    {
