...
public class AllUserInvitationsChangeTracker implements Cancellable {
  ...
  private class AllUserInvitationsInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllUserInvitationsInput(AllUserInvitationsRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllUserInvitations\\\" is required, but got value null.");
      }
    }
  ...
  private AllUserInvitations root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllUserInvitationsInput inputs;
  private Field field;

  public AllUserInvitationsChangeTracker(
  ...
  public void init(
      OutObject out, AllUserInvitations initialData, AllUserInvitationsRequest inputs) {
    {
    ...
    this.inputs = new AllUserInvitationsInput(inputs);
  ...
  private void storeInitialData(AllUserInvitations initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getExpiresAt(), x.getId()))
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyUserInvitation(((UserInvitation) obj), ((UserInvitation) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyUserInvitation(UserInvitation model, UserInvitation old, StoreEventType type) {
  ...
  private void createInsertChange(UserInvitation model) {
    long id = model.getId();
  ...
  private void createDeleteChange(UserInvitation model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(UserInvitation model, UserInvitation old) {
  ...
  private void moveItems(UserInvitation model, TypeAndId data, LocalDateTime OrderByValue0) {
    // Find old index
