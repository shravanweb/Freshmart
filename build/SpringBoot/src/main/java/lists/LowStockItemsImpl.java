...
import classes.LowStockItemsIn;
...
@Service
public class LowStockItemsImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public LowStockItemsRequest inputToRequest(LowStockItemsIn inputs) {
    LowStockItemsRequest request = new LowStockItemsRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public LowStockItems get(LowStockItemsIn inputs) {
    LowStockItemsRequest request = inputToRequest(inputs);
    return get(request);
  }

  public LowStockItems get(LowStockItemsRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public LowStockItems getAsStruct(List<NativeObj> rows, long count) {
    ...
    LowStockItems wrap = new LowStockItems();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, LowStockItemsIn inputs) throws Exception {
    LowStockItemsRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, LowStockItemsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, LowStockItemsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  }
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.LowStockItems);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(LowStockItemsRequest request) {
    String sql =
        "select a._organization_id a0, a._quantity_on_hand a1, a._id a2 from _warehouse_stock a where a._organization_id = :param_0 order by a._quantity_on_hand";
  ...
  public long getCountResult(LowStockItemsRequest request) {
    String sql =
        "select count(a._id) a0 from _warehouse_stock a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
