...
import classes.AllWarehousesIn;
...
@Service
public class AllWarehousesImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllWarehousesRequest inputToRequest(AllWarehousesIn inputs) {
    AllWarehousesRequest request = new AllWarehousesRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllWarehouses get(AllWarehousesIn inputs) {
    AllWarehousesRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllWarehouses get(AllWarehousesRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllWarehouses getAsStruct(List<NativeObj> rows, long count) {
    List<Warehouse> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.Warehouse));
    }
    AllWarehouses wrap = new AllWarehouses();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllWarehousesIn inputs) throws Exception {
    AllWarehousesRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllWarehousesRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllWarehousesRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  }
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllWarehouses);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllWarehousesRequest request) {
    String sql =
        "select a._organization_id a0, a._name a1, a._id a2 from _warehouse a where a._organization_id = :param_0 order by a._name";
  ...
  public long getCountResult(AllWarehousesRequest request) {
    String sql = "select count(a._id) a0 from _warehouse a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
