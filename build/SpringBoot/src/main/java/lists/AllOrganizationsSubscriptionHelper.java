...
public class AllOrganizationsSubscriptionHelper
  ...
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allOrganizationsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allOrganizationsImpl.getNativeResult();
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, Organization model, Organization old) {
  ...
  private List<NativeObj> createOrganizationData(Organization organization) {
    ...
    row.set(0, organization.getName());
    row.set(1, organization.getId());
    row.setId(1);
    data.add(row);
    return data;
  }
