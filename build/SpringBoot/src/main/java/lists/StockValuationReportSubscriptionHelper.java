...
public class StockValuationReportSubscriptionHelper
  ...
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        double OrderByValue0 = wrappedBase.getDouble(2);
  ...
  private Field field;
  private StockValuationReportRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = stockValuationReportImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = stockValuationReportImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, StockValuationReportRequest inputs) {
    {
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, WarehouseStock model) {
    ...
    long index = this.output.getPath(model.getStockValue());
  ...
  private List<NativeObj> createWarehouseStockData(WarehouseStock warehouseStock) {
    ...
    row.set(1, warehouseStock.getWarehouse().getId());
    row.set(2, warehouseStock.getStockValue());
    row.set(3, warehouseStock.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
