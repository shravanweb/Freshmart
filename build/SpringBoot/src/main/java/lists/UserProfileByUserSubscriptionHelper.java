...
public class UserProfileByUserSubscriptionHelper
  ...
  private Field field;
  private UserProfileByUserRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = userProfileByUserImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = userProfileByUserImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, UserProfileByUserRequest inputs) {
    {
  ...
  private List<NativeObj> createUserProfileData(UserProfile userProfile) {
    ...
    row.set(0, userProfile.getUser().getId());
    row.set(1, userProfile.getId());
    row.setId(1);
    data.add(row);
    return data;
  }
