...
public class AllStockAlertsChangeTracker implements Cancellable {
  ...
  private class AllStockAlertsInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllStockAlertsInput(AllStockAlertsRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllStockAlerts\\\" is required, but got value null.");
      }
    }
  ...
  private AllStockAlerts root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllStockAlertsInput inputs;
  private Field field;

  public AllStockAlertsChangeTracker(
  ...
  public void init(OutObject out, AllStockAlerts initialData, AllStockAlertsRequest inputs) {
    {
    ...
    this.inputs = new AllStockAlertsInput(inputs);
  ...
  private void storeInitialData(AllStockAlerts initialData) {
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyStockAlert(((StockAlert) obj), ((StockAlert) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyStockAlert(StockAlert model, StockAlert old, StoreEventType type) {
  ...
  private void createInsertChange(StockAlert model) {
    long id = model.getId();
  ...
  private void createDeleteChange(StockAlert model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(StockAlert model, StockAlert old) {
  ...
  private void moveItems(StockAlert model, TypeAndId data, LocalDateTime OrderByValue0) {
    // Find old index
