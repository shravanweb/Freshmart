...
public class ProductSearchSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        NativeObj base = wrappedBase.getRef(6);
        String OrderByValue0 = wrappedBase.getString(3);
  ...
  private Field field;
  private ProductSearchRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = productSearchImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = productSearchImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, ProductSearchRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<Product>>) subscription.onProductChangeEvent())
            .subscribe((e) -> applyProduct(e));
    disposables.add(baseSubscribe);
  }

  public void applyProduct(D3ESubscriptionEvent<Product> e) {
  ...
  private boolean applyWhere(Product model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization())
        && (inputs.getCategory() == null
            || Objects.equals(model.getCategory(), inputs.getCategory()))
        && (inputs.getStatus() == null || model.getStatus() == inputs.getStatus())
        && (inputs.getSearchTerm() == null
            || Objects.equals(inputs.getSearchTerm(), "")
            || StringExt.contains(model.getName(), inputs.getSearchTerm(), 0l)
            || StringExt.contains(model.getSku(), inputs.getSearchTerm(), 0l)
            || (model.getBarcode() != null
                && StringExt.contains(model.getBarcode(), inputs.getSearchTerm(), 0l)));
  }

  private void createInsertChange(List<DataQueryDataChange> changes, Product model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createProductData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, Product model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, Product model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, Product model, Product old) {
  ...
  private List<NativeObj> createProductData(Product product) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(7);
    row.set(0, product.getOrganization().getId());
    row.set(1, product.getCategory().getId());
    row.set(2, product.getStatus());
    row.set(3, product.getName());
    row.set(4, product.getSku());
    row.set(5, product.getBarcode());
    row.set(6, product.getId());
    row.setId(6);
    data.add(row);
    return data;
  }
