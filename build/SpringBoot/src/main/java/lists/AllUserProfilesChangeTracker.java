...
public class AllUserProfilesChangeTracker implements Cancellable {
  ...
  private class AllUserProfilesInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllUserProfilesInput(AllUserProfilesRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllUserProfiles\\\" is required, but got value null.");
      }
    }
  ...
  private AllUserProfiles root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllUserProfilesInput inputs;
  private Field field;

  public AllUserProfilesChangeTracker(
  ...
  public void init(OutObject out, AllUserProfiles initialData, AllUserProfilesRequest inputs) {
    {
    ...
    this.inputs = new AllUserProfilesInput(inputs);
  ...
  private void storeInitialData(AllUserProfiles initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getDisplayName(), x.getId()))
  ...
  private boolean createPathChangeChange(UserProfile model, UserProfile old) {
  ...
  private void moveItems(UserProfile model, TypeAndId data, String OrderByValue0) {
    // Find old index
