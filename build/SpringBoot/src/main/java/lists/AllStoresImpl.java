...
@Service
public class AllStoresImpl extends AbsDataQueryImpl {
  ...
  public AllStoresRequest inputToRequest(AllStoresIn inputs) {
    AllStoresRequest request = new AllStoresRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllStores get(AllStoresIn inputs) {
    AllStoresRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllStores get(AllStoresRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllStores getAsStruct(List<NativeObj> rows, long count) {
    List<Store> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.Store));
    }
    AllStores wrap = new AllStores();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllStoresIn inputs) throws Exception {
    AllStoresRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllStoresRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public JSONObject getAsJson(Field field, List<NativeObj> rows, long count) throws Exception {
    ...
    for (NativeObj _r1 : rows) {
      Store store = NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.Store);
      em.get().unproxy(store);
      array.put(fetcher.fetch(field, "Store", store));
    }
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllStoresRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  }
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllStores);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllStoresRequest request) {
    String sql =
        "select a._organization_id a0, a._name a1, a._id a2 from _store a where a._organization_id = :param_0 order by a._name";
  ...
  public long getCountResult(AllStoresRequest request) {
    String sql = "select count(a._id) a0 from _store a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
