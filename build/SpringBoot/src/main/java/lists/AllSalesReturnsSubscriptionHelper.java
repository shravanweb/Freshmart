...
public class AllSalesReturnsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllSalesReturnsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allSalesReturnsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allSalesReturnsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllSalesReturnsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<SalesReturn>>) subscription.onSalesReturnChangeEvent())
            .subscribe((e) -> applySalesReturn(e));
    disposables.add(baseSubscribe);
  }

  public void applySalesReturn(D3ESubscriptionEvent<SalesReturn> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    SalesReturn model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, SalesReturn model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createSalesReturnData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getReturnDate());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, SalesReturn model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, SalesReturn model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, SalesReturn model, SalesReturn old) {
  ...
  private List<NativeObj> createSalesReturnData(SalesReturn salesReturn) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, salesReturn.getOrganization().getId());
    row.set(1, salesReturn.getReturnDate());
    row.set(2, salesReturn.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
