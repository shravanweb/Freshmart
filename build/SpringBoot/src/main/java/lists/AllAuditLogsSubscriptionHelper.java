...
public class AllAuditLogsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllAuditLogsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allAuditLogsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allAuditLogsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllAuditLogsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<AuditLog>>) subscription.onAuditLogChangeEvent())
            .subscribe((e) -> applyAuditLog(e));
    disposables.add(baseSubscribe);
  }

  public void applyAuditLog(D3ESubscriptionEvent<AuditLog> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    AuditLog model = e.model;
  ...
  private boolean applyWhere(AuditLog model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, AuditLog model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createAuditLogData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getPerformedAt());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, AuditLog model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, AuditLog model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, AuditLog model, AuditLog old) {
    LocalDateTime oldValue0 = old.getPerformedAt();
    LocalDateTime modelValue0 = model.getPerformedAt();
    ...
    LocalDateTime OrderByValue0 = model.getPerformedAt();
  ...
  private List<NativeObj> createAuditLogData(AuditLog auditLog) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, auditLog.getOrganization().getId());
    row.set(1, auditLog.getPerformedAt());
    row.set(2, auditLog.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
