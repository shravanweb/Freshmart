...
public class AllGoodsReceiptsChangeTracker implements Cancellable {
  ...
  private class AllGoodsReceiptsInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllGoodsReceiptsInput(AllGoodsReceiptsRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllGoodsReceipts\\\" is required, but got value null.");
      }
    }
  ...
  private AllGoodsReceipts root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllGoodsReceiptsInput inputs;
  private Field field;

  public AllGoodsReceiptsChangeTracker(
  ...
  public void init(OutObject out, AllGoodsReceipts initialData, AllGoodsReceiptsRequest inputs) {
    {
    ...
    this.inputs = new AllGoodsReceiptsInput(inputs);
  ...
  private void storeInitialData(AllGoodsReceipts initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getReceiptDate(), x.getId()))
  ...
  private boolean createPathChangeChange(GoodsReceipt model, GoodsReceipt old) {
  ...
  private void moveItems(GoodsReceipt model, TypeAndId data, LocalDate OrderByValue0) {
    // Find old index
