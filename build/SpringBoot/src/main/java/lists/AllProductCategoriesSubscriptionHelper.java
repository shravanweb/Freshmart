...
public class AllProductCategoriesSubscriptionHelper
  ...
  private Field field;
  private AllProductCategoriesRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allProductCategoriesImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allProductCategoriesImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllProductCategoriesRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<ProductCategory>>)
                subscription.onProductCategoryChangeEvent())
            .subscribe((e) -> applyProductCategory(e));
    disposables.add(baseSubscribe);
  }

  public void applyProductCategory(D3ESubscriptionEvent<ProductCategory> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    ProductCategory model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      ProductCategory old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private boolean applyWhere(ProductCategory model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, ProductCategory model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createProductCategoryData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, ProductCategory model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, ProductCategory model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, ProductCategory model, ProductCategory old) {
  ...
  private List<NativeObj> createProductCategoryData(ProductCategory productCategory) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, productCategory.getOrganization().getId());
    row.set(1, productCategory.getName());
    row.set(2, productCategory.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
