...
public class AllInventoryAdjustmentsSubscriptionHelper
  ...
  private Field field;
  private AllInventoryAdjustmentsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allInventoryAdjustmentsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allInventoryAdjustmentsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(
      Field field, AllInventoryAdjustmentsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<InventoryAdjustment>>)
                subscription.onInventoryAdjustmentChangeEvent())
            .subscribe((e) -> applyInventoryAdjustment(e));
    disposables.add(baseSubscribe);
  }

  public void applyInventoryAdjustment(D3ESubscriptionEvent<InventoryAdjustment> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    InventoryAdjustment model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      InventoryAdjustment old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private boolean applyWhere(InventoryAdjustment model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, InventoryAdjustment model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createInventoryAdjustmentData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getAdjustmentDate());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, InventoryAdjustment model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, InventoryAdjustment model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, InventoryAdjustment model, InventoryAdjustment old) {
    LocalDate oldValue0 = old.getAdjustmentDate();
    LocalDate modelValue0 = model.getAdjustmentDate();
    ...
    LocalDate OrderByValue0 = model.getAdjustmentDate();
  ...
  private List<NativeObj> createInventoryAdjustmentData(InventoryAdjustment inventoryAdjustment) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, inventoryAdjustment.getOrganization().getId());
    row.set(1, inventoryAdjustment.getAdjustmentDate());
    row.set(2, inventoryAdjustment.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
