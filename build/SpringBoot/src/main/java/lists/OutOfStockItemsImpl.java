...
import classes.OutOfStockItemsIn;
...
@Service
public class OutOfStockItemsImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public OutOfStockItemsRequest inputToRequest(OutOfStockItemsIn inputs) {
    OutOfStockItemsRequest request = new OutOfStockItemsRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public OutOfStockItems get(OutOfStockItemsIn inputs) {
    OutOfStockItemsRequest request = inputToRequest(inputs);
    return get(request);
  }

  public OutOfStockItems get(OutOfStockItemsRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public OutOfStockItems getAsStruct(List<NativeObj> rows, long count) {
    ...
    OutOfStockItems wrap = new OutOfStockItems();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, OutOfStockItemsIn inputs) throws Exception {
    OutOfStockItemsRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, OutOfStockItemsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, OutOfStockItemsRequest request)
      throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.OutOfStockItems);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(OutOfStockItemsRequest request) {
    String sql =
        "select a._organization_id a0, a._quantity_on_hand a1, a._id a2 from _warehouse_stock a where a._organization_id = :param_0 and a._quantity_on_hand = 0.0 order by a._quantity_on_hand";
  ...
  public long getCountResult(OutOfStockItemsRequest request) {
    String sql =
        "select count(a._id) a0 from _warehouse_stock a where a._organization_id = :param_0 and a._quantity_on_hand = 0.0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
