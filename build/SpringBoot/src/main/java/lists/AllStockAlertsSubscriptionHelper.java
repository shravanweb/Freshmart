...
public class AllStockAlertsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllStockAlertsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allStockAlertsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allStockAlertsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllStockAlertsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<StockAlert>>) subscription.onStockAlertChangeEvent())
            .subscribe((e) -> applyStockAlert(e));
    disposables.add(baseSubscribe);
  }

  public void applyStockAlert(D3ESubscriptionEvent<StockAlert> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    StockAlert model = e.model;
  ...
  private boolean applyWhere(StockAlert model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, StockAlert model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createStockAlertData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, StockAlert model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, StockAlert model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, StockAlert model, StockAlert old) {
  ...
  private List<NativeObj> createStockAlertData(StockAlert stockAlert) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, stockAlert.getOrganization().getId());
    row.set(1, stockAlert.getCreatedAt());
    row.set(2, stockAlert.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
