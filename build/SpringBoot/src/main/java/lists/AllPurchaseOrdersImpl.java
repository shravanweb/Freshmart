...
import classes.AllPurchaseOrdersIn;
...
@Service
public class AllPurchaseOrdersImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllPurchaseOrdersRequest inputToRequest(AllPurchaseOrdersIn inputs) {
    AllPurchaseOrdersRequest request = new AllPurchaseOrdersRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllPurchaseOrders get(AllPurchaseOrdersIn inputs) {
    AllPurchaseOrdersRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllPurchaseOrders get(AllPurchaseOrdersRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllPurchaseOrders getAsStruct(List<NativeObj> rows, long count) {
    List<PurchaseOrder> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.PurchaseOrder));
    }
    AllPurchaseOrders wrap = new AllPurchaseOrders();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllPurchaseOrdersIn inputs) throws Exception {
    AllPurchaseOrdersRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllPurchaseOrdersRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllPurchaseOrdersRequest request)
      throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllPurchaseOrders);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllPurchaseOrdersRequest request) {
    String sql =
        "select a._organization_id a0, a._order_date a1, a._id a2 from _purchase_order a where a._organization_id = :param_0 order by a._order_date desc";
  ...
  public long getCountResult(AllPurchaseOrdersRequest request) {
    String sql =
        "select count(a._id) a0 from _purchase_order a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
