...
public class UnreadNotificationCountSubscriptionHelper
  ...
  private Field field;
  private UnreadNotificationCountRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = unreadNotificationCountImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = unreadNotificationCountImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(
      Field field, UnreadNotificationCountRequest inputs) {
    {
  ...
  private List<NativeObj> createInAppNotificationData(InAppNotification inAppNotification) {
    ...
    row.set(2, inAppNotification.isIsRead());
