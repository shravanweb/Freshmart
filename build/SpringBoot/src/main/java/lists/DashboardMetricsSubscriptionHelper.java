...
public class DashboardMetricsSubscriptionHelper
  ...
  private Field field;
  private DashboardMetricsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = dashboardMetricsImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = dashboardMetricsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, DashboardMetricsRequest inputs) {
    {
  ...
  private List<NativeObj> createWarehouseStockData(WarehouseStock warehouseStock) {
    ...
    row.set(1, warehouseStock.getStockValue());
