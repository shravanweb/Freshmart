...
public class AllInAppNotificationsSubscriptionHelper
  ...
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        LocalDateTime OrderByValue0 = wrappedBase.getDateTime(2);
  ...
  private Field field;
  private AllInAppNotificationsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allInAppNotificationsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allInAppNotificationsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllInAppNotificationsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<InAppNotification>>)
                subscription.onInAppNotificationChangeEvent())
            .subscribe((e) -> applyInAppNotification(e));
    disposables.add(baseSubscribe);
  }

  public void applyInAppNotification(D3ESubscriptionEvent<InAppNotification> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    InAppNotification model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      InAppNotification old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, InAppNotification model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createInAppNotificationData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getCreatedAt());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, InAppNotification model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, InAppNotification model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, InAppNotification model, InAppNotification old) {
  ...
  private List<NativeObj> createInAppNotificationData(InAppNotification inAppNotification) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(4);
    row.set(0, inAppNotification.getOrganization().getId());
    row.set(1, inAppNotification.getRecipient().getId());
    row.set(2, inAppNotification.getCreatedAt());
    row.set(3, inAppNotification.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
