...
public class ProductsByCategorySubscriptionHelper
  ...
  private Field field;
  private ProductsByCategoryRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = productsByCategoryImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = productsByCategoryImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, ProductsByCategoryRequest inputs) {
    {
  ...
  private List<NativeObj> createProductData(Product product) {
    ...
    row.set(2, product.getName());
    row.set(3, product.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
