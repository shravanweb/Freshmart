...
public class PurchaseOrdersByStatusSubscriptionHelper
  ...
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        LocalDate OrderByValue0 = wrappedBase.getDate(2);
  ...
  private Field field;
  private PurchaseOrdersByStatusRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = purchaseOrdersByStatusImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = purchaseOrdersByStatusImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(
      Field field, PurchaseOrdersByStatusRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<PurchaseOrder>>) subscription.onPurchaseOrderChangeEvent())
            .subscribe((e) -> applyPurchaseOrder(e));
    disposables.add(baseSubscribe);
  }

  public void applyPurchaseOrder(D3ESubscriptionEvent<PurchaseOrder> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    PurchaseOrder model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      PurchaseOrder old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, PurchaseOrder model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createPurchaseOrderData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, PurchaseOrder model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, PurchaseOrder model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, PurchaseOrder model, PurchaseOrder old) {
  ...
  private List<NativeObj> createPurchaseOrderData(PurchaseOrder purchaseOrder) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(4);
    row.set(0, purchaseOrder.getOrganization().getId());
    row.set(1, purchaseOrder.getStatus());
    row.set(2, purchaseOrder.getOrderDate());
    row.set(3, purchaseOrder.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
