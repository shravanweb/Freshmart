...
public class AllPurchaseOrdersSubscriptionHelper
  ...
  private Field field;
  private AllPurchaseOrdersRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allPurchaseOrdersImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allPurchaseOrdersImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllPurchaseOrdersRequest inputs) {
    {
  ...
  private List<NativeObj> createPurchaseOrderData(PurchaseOrder purchaseOrder) {
    ...
    row.set(1, purchaseOrder.getOrderDate());
    row.set(2, purchaseOrder.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
