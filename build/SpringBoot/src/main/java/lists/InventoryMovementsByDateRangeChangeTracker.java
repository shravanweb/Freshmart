...
public class InventoryMovementsByDateRangeChangeTracker implements Cancellable {
  ...
  private class InventoryMovementsByDateRangeInput {
    private TypeAndId organizationRef;
    private Organization organization;
    private LocalDateTime startDate;
    private LocalDateTime endDate;

    public InventoryMovementsByDateRangeInput(InventoryMovementsByDateRangeRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"InventoryMovementsByDateRange\\\" is required, but got value null.");
      }
      this.startDate = _req.getStartDate();
      this.endDate = _req.getEndDate();
    }
  ...
  private InventoryMovementsByDateRange root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private InventoryMovementsByDateRangeInput inputs;
  private Field field;

  public InventoryMovementsByDateRangeChangeTracker(
  ...
  public void init(
      OutObject out,
      InventoryMovementsByDateRange initialData,
    ...
    this.inputs = new InventoryMovementsByDateRangeInput(inputs);
  ...
  private void storeInitialData(InventoryMovementsByDateRange initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getMovementDate(), x.getId()))
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyInventoryMovement(((InventoryMovement) obj), ((InventoryMovement) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyInventoryMovement(
      InventoryMovement model, InventoryMovement old, StoreEventType type) {
  ...
  private void createInsertChange(InventoryMovement model) {
    long id = model.getId();
  ...
  private void createDeleteChange(InventoryMovement model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(InventoryMovement model, InventoryMovement old) {
  ...
  private void moveItems(InventoryMovement model, TypeAndId data, LocalDateTime OrderByValue0) {
    // Find old index
