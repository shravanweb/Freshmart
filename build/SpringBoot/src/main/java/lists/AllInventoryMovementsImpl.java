...
import classes.AllInventoryMovementsIn;
...
@Service
public class AllInventoryMovementsImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllInventoryMovementsRequest inputToRequest(AllInventoryMovementsIn inputs) {
    AllInventoryMovementsRequest request = new AllInventoryMovementsRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllInventoryMovements get(AllInventoryMovementsIn inputs) {
    AllInventoryMovementsRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllInventoryMovements get(AllInventoryMovementsRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllInventoryMovements getAsStruct(List<NativeObj> rows, long count) {
    ...
    AllInventoryMovements wrap = new AllInventoryMovements();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllInventoryMovementsIn inputs) throws Exception {
    AllInventoryMovementsRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllInventoryMovementsRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllInventoryMovementsRequest request)
      throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllInventoryMovements);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllInventoryMovementsRequest request) {
    String sql =
        "select a._organization_id a0, a._movement_date a1, a._id a2 from _inventory_movement a where a._organization_id = :param_0 order by a._movement_date desc";
  ...
  public long getCountResult(AllInventoryMovementsRequest request) {
    String sql =
        "select count(a._id) a0 from _inventory_movement a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
