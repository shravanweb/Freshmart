...
public class NativeObj {
	...
	public Geolocation getGeolocation(int index) {
		...
		Geolocation loc = new Geolocation(Double.parseDouble(split[1]), Double.parseDouble(split[0]));
		return loc;
	}
	...
	public boolean equals(Object obj) {
		if(obj instanceof NativeObj) {
			long _id = ((NativeObj) obj).getId();
			return _id == getId();
		} else if(obj instanceof DatabaseObject) {
			long _id = ((DatabaseObject) obj).getId();
			return _id == getId();
		}
		return false;
	}
	...
	public List<LocalTime> getListTime(int index) {
        String ids = (String) row[index];
        List<LocalTime> allTimes = Arrays.stream(ids.substring(1, ids.length() - 1).split(","))
                .filter(i -> !i.equals("NULL"))
                .map(i -> LocalTime.parse(i.substring(1, i.length() - 1), dtf))
                .collect(Collectors.toList());
        return allTimes;
    }
}
