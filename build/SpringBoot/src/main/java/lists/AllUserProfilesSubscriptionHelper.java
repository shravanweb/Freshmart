...
public class AllUserProfilesSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        String OrderByValue0 = wrappedBase.getString(1);
  ...
  private Field field;
  private AllUserProfilesRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allUserProfilesImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allUserProfilesImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllUserProfilesRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<UserProfile>>) subscription.onUserProfileChangeEvent())
            .subscribe((e) -> applyUserProfile(e));
    disposables.add(baseSubscribe);
  }

  public void applyUserProfile(D3ESubscriptionEvent<UserProfile> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    UserProfile model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, UserProfile model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createUserProfileData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getDisplayName());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, UserProfile model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, UserProfile model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, UserProfile model, UserProfile old) {
    String oldValue0 = old.getDisplayName();
    String modelValue0 = model.getDisplayName();
    ...
    String OrderByValue0 = model.getDisplayName();
  ...
  private List<NativeObj> createUserProfileData(UserProfile userProfile) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, userProfile.getOrganization().getId());
    row.set(1, userProfile.getDisplayName());
    row.set(2, userProfile.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
