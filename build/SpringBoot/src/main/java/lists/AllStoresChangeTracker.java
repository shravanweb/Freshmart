...
public class AllStoresChangeTracker implements Cancellable {
  ...
  private class AllStoresInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllStoresInput(AllStoresRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllStores\\\" is required, but got value null.");
      }
    }
  ...
  public void init(OutObject out, AllStores initialData, AllStoresRequest inputs) {
    {
    ...
    this.inputs = new AllStoresInput(inputs);
  ...
  private void storeInitialData(AllStores initialData) {
  ...
  private boolean createPathChangeChange(Store model, Store old) {
  ...
  private void moveItems(Store model, TypeAndId data, String OrderByValue0) {
    // Find old index
