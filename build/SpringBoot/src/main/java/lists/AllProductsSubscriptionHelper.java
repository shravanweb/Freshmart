...
public class AllProductsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllProductsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allProductsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allProductsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllProductsRequest inputs) {
    {
  ...
  private List<NativeObj> createProductData(Product product) {
    ...
    row.set(1, product.getName());
    row.set(2, product.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
