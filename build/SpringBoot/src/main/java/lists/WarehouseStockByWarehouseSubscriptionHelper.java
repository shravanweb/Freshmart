...
public class WarehouseStockByWarehouseSubscriptionHelper
  ...
  private Field field;
  private WarehouseStockByWarehouseRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = warehouseStockByWarehouseImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = warehouseStockByWarehouseImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(
      Field field, WarehouseStockByWarehouseRequest inputs) {
    {
  ...
  private List<NativeObj> createWarehouseStockData(WarehouseStock warehouseStock) {
    ...
    row.set(0, warehouseStock.getWarehouse().getId());
