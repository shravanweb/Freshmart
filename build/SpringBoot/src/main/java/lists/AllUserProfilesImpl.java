...
import classes.AllUserProfilesIn;
...
@Service
public class AllUserProfilesImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private OrganizationRepository organizationRepository;

  public AllUserProfilesRequest inputToRequest(AllUserProfilesIn inputs) {
    AllUserProfilesRequest request = new AllUserProfilesRequest();
    request.setOrganization(organizationRepository.findById(inputs.organization));
    return request;
  }

  public AllUserProfiles get(AllUserProfilesIn inputs) {
    AllUserProfilesRequest request = inputToRequest(inputs);
    return get(request);
  }

  public AllUserProfiles get(AllUserProfilesRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public AllUserProfiles getAsStruct(List<NativeObj> rows, long count) {
    List<UserProfile> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em.get(), _r1.getRef(2), SchemaConstants.UserProfile));
    }
    AllUserProfiles wrap = new AllUserProfiles();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, AllUserProfilesIn inputs) throws Exception {
    AllUserProfilesRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, AllUserProfilesRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, AllUserProfilesRequest request)
      throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.AllUserProfiles);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(AllUserProfilesRequest request) {
    String sql =
        "select a._organization_id a0, a._display_name a1, a._id a2 from _user_profile a where a._organization_id = :param_0 order by a._display_name";
  ...
  public long getCountResult(AllUserProfilesRequest request) {
    String sql = "select count(a._id) a0 from _user_profile a where a._organization_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getOrganization());
    this.logQuery(sql, query);
