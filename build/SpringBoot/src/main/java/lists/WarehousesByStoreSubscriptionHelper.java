...
public class WarehousesByStoreSubscriptionHelper
  ...
  private Field field;
  private WarehousesByStoreRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = warehousesByStoreImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = warehousesByStoreImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, WarehousesByStoreRequest inputs) {
    {
  ...
  private List<NativeObj> createWarehouseData(Warehouse warehouse) {
    ...
    row.set(0, warehouse.getStore().getId());
