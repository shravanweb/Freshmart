...
public class AllSuppliersSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllSuppliersRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allSuppliersImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allSuppliersImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllSuppliersRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<Vendor>>) subscription.onVendorChangeEvent())
            .subscribe((e) -> applyVendor(e));
    disposables.add(baseSubscribe);
  }

  public void applyVendor(D3ESubscriptionEvent<Vendor> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    Vendor model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, Vendor model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createVendorData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, Vendor model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, Vendor model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, Vendor model, Vendor old) {
  ...
  private List<NativeObj> createVendorData(Vendor vendor) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, vendor.getOrganization().getId());
    row.set(1, vendor.getName());
    row.set(2, vendor.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
