...
public class AllAuditLogsChangeTracker implements Cancellable {
  ...
  private class AllAuditLogsInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllAuditLogsInput(AllAuditLogsRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllAuditLogs\\\" is required, but got value null.");
      }
    }
  ...
  public void init(OutObject out, AllAuditLogs initialData, AllAuditLogsRequest inputs) {
    {
    ...
    this.inputs = new AllAuditLogsInput(inputs);
  ...
  private void storeInitialData(AllAuditLogs initialData) {
    this.root = initialData;
    this.orderBy =
        initialData.getItems().stream()
            .map((x) -> new OrderBy(x.getPerformedAt(), x.getId()))
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyAuditLog(((AuditLog) obj), ((AuditLog) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyAuditLog(AuditLog model, AuditLog old, StoreEventType type) {
  ...
  private void createInsertChange(AuditLog model) {
    long id = model.getId();
  ...
  private void createDeleteChange(AuditLog model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(AuditLog model, AuditLog old) {
  ...
  private void moveItems(AuditLog model, TypeAndId data, LocalDateTime OrderByValue0) {
    // Find old index
