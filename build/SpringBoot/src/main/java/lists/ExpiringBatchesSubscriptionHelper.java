...
public class ExpiringBatchesSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private ExpiringBatchesRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = expiringBatchesImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = expiringBatchesImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, ExpiringBatchesRequest inputs) {
    {
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, StockBatch model) {
    ...
    long index = this.output.getPath(model.getExpiryDate());
  ...
  private List<NativeObj> createStockBatchData(StockBatch stockBatch) {
    ...
    row.set(1, stockBatch.getExpiryDate());
