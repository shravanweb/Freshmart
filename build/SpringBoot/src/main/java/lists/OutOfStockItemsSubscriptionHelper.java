...
public class OutOfStockItemsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  private class Output {
    ...
    public Output(List<NativeObj> rows) {
      for (int i = 0; i < rows.size(); i++) {
        ...
        double OrderByValue0 = wrappedBase.getDouble(1);
    ...
    public long getPath(double OrderByValue0) {
      long index = 0;
  ...
  private class OrderBy {
    ...
    public OrderBy(double OrderByValue0, Row row) {
  ...
  private Field field;
  private OutOfStockItemsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = outOfStockItemsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = outOfStockItemsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, OutOfStockItemsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<WarehouseStock>>)
                subscription.onWarehouseStockChangeEvent())
            .subscribe((e) -> applyWarehouseStock(e));
    disposables.add(baseSubscribe);
  }

  public void applyWarehouseStock(D3ESubscriptionEvent<WarehouseStock> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    WarehouseStock model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      WarehouseStock old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private boolean applyWhere(WarehouseStock model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization())
        && model.getQuantityOnHand() == 0.0d;
  }

  private void createInsertChange(List<DataQueryDataChange> changes, WarehouseStock model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createWarehouseStockData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getQuantityOnHand());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, WarehouseStock model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, WarehouseStock model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, WarehouseStock model, WarehouseStock old) {
  ...
  private List<NativeObj> createWarehouseStockData(WarehouseStock warehouseStock) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, warehouseStock.getOrganization().getId());
    row.set(1, warehouseStock.getQuantityOnHand());
    row.set(2, warehouseStock.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
