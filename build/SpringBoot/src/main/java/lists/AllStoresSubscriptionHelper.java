...
public class AllStoresSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllStoresRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allStoresImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allStoresImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllStoresRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<Store>>) subscription.onStoreChangeEvent())
            .subscribe((e) -> applyStore(e));
    disposables.add(baseSubscribe);
  }

  public void applyStore(D3ESubscriptionEvent<Store> e) {
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, Store model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createStoreData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, Store model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, Store model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, Store model, Store old) {
  ...
  private List<NativeObj> createStoreData(Store store) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, store.getOrganization().getId());
    row.set(1, store.getName());
    row.set(2, store.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
