...
public class SalesOrdersByStoreSubscriptionHelper
  ...
  private Field field;
  private SalesOrdersByStoreRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = salesOrdersByStoreImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = salesOrdersByStoreImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, SalesOrdersByStoreRequest inputs) {
    {
  ...
  private List<NativeObj> createSalesOrderData(SalesOrder salesOrder) {
    ...
    row.set(0, salesOrder.getStore().getId());
