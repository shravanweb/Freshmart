...
public class AllStockBatchesSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  private Field field;
  private AllStockBatchesRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allStockBatchesImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allStockBatchesImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllStockBatchesRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<StockBatch>>) subscription.onStockBatchChangeEvent())
            .subscribe((e) -> applyStockBatch(e));
    disposables.add(baseSubscribe);
  }

  public void applyStockBatch(D3ESubscriptionEvent<StockBatch> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    StockBatch model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, StockBatch model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createStockBatchData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getBatchNumber());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, StockBatch model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, StockBatch model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, StockBatch model, StockBatch old) {
  ...
  private List<NativeObj> createStockBatchData(StockBatch stockBatch) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, stockBatch.getOrganization().getId());
    row.set(1, stockBatch.getBatchNumber());
    row.set(2, stockBatch.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
