...
import classes.SalesOrdersByStoreIn;
...
@Service
public class SalesOrdersByStoreImpl extends AbsDataQueryImpl {
  @Autowired private D3EEntityManagerProvider em;
  @Autowired private GqlToSql gqlToSql;
  @Autowired private gqltosql2.GqlToSql gqlToSql2;
  @Autowired private StoreRepository storeRepository;

  public SalesOrdersByStoreRequest inputToRequest(SalesOrdersByStoreIn inputs) {
    SalesOrdersByStoreRequest request = new SalesOrdersByStoreRequest();
    request.setStore(storeRepository.findById(inputs.store));
    return request;
  }

  public SalesOrdersByStore get(SalesOrdersByStoreIn inputs) {
    SalesOrdersByStoreRequest request = inputToRequest(inputs);
    return get(request);
  }

  public SalesOrdersByStore get(SalesOrdersByStoreRequest request) {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsStruct(rows, count);
  }

  public SalesOrdersByStore getAsStruct(List<NativeObj> rows, long count) {
    ...
    SalesOrdersByStore wrap = new SalesOrdersByStore();
    wrap.setItems(result);
    wrap.setCount(count);
    return wrap;
  }

  public JSONObject getAsJson(Field field, SalesOrdersByStoreIn inputs) throws Exception {
    SalesOrdersByStoreRequest request = inputToRequest(inputs);
    return getAsJson(field, request);
  }

  public JSONObject getAsJson(Field field, SalesOrdersByStoreRequest request) throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
  ...
  public OutObject getAsJson(gqltosql2.Field field, SalesOrdersByStoreRequest request)
      throws Exception {
    List<NativeObj> rows = getNativeResult(request);
    long count = getCountResult(request);
    return getAsJson(field, rows, count);
  ...
  public OutObject getAsJson(gqltosql2.Field field, List<NativeObj> rows, long count)
    ...
    result.addType(SchemaConstants.SalesOrdersByStore);
    result.add("items", array);
    result.add("count", count);
    return result;
  }

  public List<NativeObj> getNativeResult(SalesOrdersByStoreRequest request) {
    String sql =
        "select a._store_id a0, a._order_date a1, a._id a2 from _sales_order a where a._store_id = :param_0 order by a._order_date desc";
  ...
  public long getCountResult(SalesOrdersByStoreRequest request) {
    String sql = "select count(a._id) a0 from _sales_order a where a._store_id = :param_0";
    Query query = em.get().createNativeQuery(sql);
    setDatabaseObjectParameter(query, "param_0", request.getStore());
    this.logQuery(sql, query);
