...
public class ProductSearchChangeTracker implements Cancellable {
  ...
  private class ProductSearchInput {
    private TypeAndId organizationRef;
    private Organization organization;
    private String searchTerm;
    private TypeAndId categoryRef;
    private ProductCategory category;
    private ProductStatus status;

    public ProductSearchInput(ProductSearchRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"ProductSearch\\\" is required, but got value null.");
      }
      this.searchTerm = _req.getSearchTerm();
      this.categoryRef = TypeAndId.from(_req.getCategory());
    ...
    public void load(DataChangeTracker tracker) {
      ...
      if (this.categoryRef != null) {
        this.category = tracker.fromTypeAndId(this.categoryRef);
      }
    }
  ...
  private ProductSearch root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private ProductSearchInput inputs;
  private Field field;

  public ProductSearchChangeTracker(
  ...
  public void init(OutObject out, ProductSearch initialData, ProductSearchRequest inputs) {
    {
    ...
    this.inputs = new ProductSearchInput(inputs);
  ...
  private void storeInitialData(ProductSearch initialData) {
  ...
  private boolean createPathChangeChange(Product model, Product old) {
  ...
  private void moveItems(Product model, TypeAndId data, String OrderByValue0) {
    // Find old index
