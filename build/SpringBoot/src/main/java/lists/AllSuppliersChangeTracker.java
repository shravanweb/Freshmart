...
public class AllSuppliersChangeTracker implements Cancellable {
  ...
  private class AllSuppliersInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllSuppliersInput(AllSuppliersRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllSuppliers\\\" is required, but got value null.");
      }
    }
  ...
  public void init(OutObject out, AllSuppliers initialData, AllSuppliersRequest inputs) {
    {
    ...
    this.inputs = new AllSuppliersInput(inputs);
  ...
  private void storeInitialData(AllSuppliers initialData) {
  ...
  private boolean createPathChangeChange(Vendor model, Vendor old) {
  ...
  private void moveItems(Vendor model, TypeAndId data, String OrderByValue0) {
    // Find old index
