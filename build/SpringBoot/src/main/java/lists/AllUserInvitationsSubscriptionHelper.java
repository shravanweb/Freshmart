...
public class AllUserInvitationsSubscriptionHelper
  ...
  private Field field;
  private AllUserInvitationsRequest inputs;

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allUserInvitationsImpl.getAsJson(field, event.nativeData, event.count);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allUserInvitationsImpl.getNativeResult(this.inputs);
  ...
  public Flowable<DataQueryDataChange> subscribe(Field field, AllUserInvitationsRequest inputs) {
    {
  ...
  private void addSubscriptions() {
    ...
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<UserInvitation>>)
                subscription.onUserInvitationChangeEvent())
            .subscribe((e) -> applyUserInvitation(e));
    disposables.add(baseSubscribe);
  }

  public void applyUserInvitation(D3ESubscriptionEvent<UserInvitation> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    UserInvitation model = e.model;
    ...
    if (type == StoreEventType.Insert) {
      ...
      UserInvitation old = model.getOld();
      if (old == null) {
        return;
      }
  ...
  private boolean applyWhere(UserInvitation model) {
    return Objects.equals(model.getOrganization(), inputs.getOrganization());
  }

  private void createInsertChange(List<DataQueryDataChange> changes, UserInvitation model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createUserInvitationData(model);
    change.changeType = SubscriptionChangeType.Insert;
    long index = this.output.getPath(model.getExpiresAt());
    change.path = index == output.rows.size() ? "-1" : Long.toString(index);
    change.index = output.rows.size();
    changes.add(change);
  }

  private void createUpdateChange(List<DataQueryDataChange> changes, UserInvitation model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, UserInvitation model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private boolean createPathChangeChange(
      List<DataQueryDataChange> changes, UserInvitation model, UserInvitation old) {
    LocalDateTime oldValue0 = old.getExpiresAt();
    LocalDateTime modelValue0 = model.getExpiresAt();
    ...
    LocalDateTime OrderByValue0 = model.getExpiresAt();
  ...
  private List<NativeObj> createUserInvitationData(UserInvitation userInvitation) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(3);
    row.set(0, userInvitation.getOrganization().getId());
    row.set(1, userInvitation.getExpiresAt());
    row.set(2, userInvitation.getId());
    row.setId(2);
    data.add(row);
    return data;
  }
