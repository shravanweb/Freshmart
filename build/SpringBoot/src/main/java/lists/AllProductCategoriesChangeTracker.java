...
public class AllProductCategoriesChangeTracker implements Cancellable {
  ...
  private class AllProductCategoriesInput {
    private TypeAndId organizationRef;
    private Organization organization;

    public AllProductCategoriesInput(AllProductCategoriesRequest _req) {
      this.organizationRef = TypeAndId.from(_req.getOrganization());
      if (_req.getOrganization() == null) {
        Log.info(
            "WARNING: Input \\\"organization\\\" in \\\"AllProductCategories\\\" is required, but got value null.");
      }
    }
  ...
  private AllProductCategories root;
  private List<OrderBy> orderBy = ListExt.List();
  private DataChangeTracker tracker;
  private ClientSession clientSession;
  private List<Disposable> disposables = ListExt.List();
  private AllProductCategoriesInput inputs;
  private Field field;

  public AllProductCategoriesChangeTracker(
  ...
  public void init(
      OutObject out, AllProductCategories initialData, AllProductCategoriesRequest inputs) {
    {
    ...
    this.inputs = new AllProductCategoriesInput(inputs);
  ...
  private void storeInitialData(AllProductCategories initialData) {
    ...
    Disposable baseSubscribe =
        tracker.listen(
            ...
            (obj, old, type) -> {
              try {
                inputs.load(tracker);
                applyProductCategory(((ProductCategory) obj), ((ProductCategory) old), type);
              } finally {
                inputs.unload();
              }
            });
  ...
  public void applyProductCategory(
      ProductCategory model, ProductCategory old, StoreEventType type) {
  ...
  private void createInsertChange(ProductCategory model) {
    long id = model.getId();
  ...
  private void createDeleteChange(ProductCategory model) {
    long id = model.getId();
  ...
  private boolean createPathChangeChange(ProductCategory model, ProductCategory old) {
  ...
  private void moveItems(ProductCategory model, TypeAndId data, String OrderByValue0) {
    // Find old index
