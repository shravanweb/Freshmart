...
export default class AllProducts extends DBObject {
  ...
  private _items: Array<Product> = ListWrapper.vanilla(
    this,
    "items",
    AllProducts._ITEMS
  );
  ...
  public setStatus(val: ResultStatus): void {
    ...
    this.updateD3EChanges(AllProducts._STATUS, this._status.index);
  ...
  public setErrors(val: Array<string>): void {
    ...
    if (!this.d3eChanges.contains(AllProducts._ERRORS)) {
      let _old: Array<string> = Array.from(this._errors);

      this.updateD3EChanges(AllProducts._ERRORS, _old);
    }
  ...
  public addToErrors(val: string, index: number = -1): void {
    ...
    let _isNewChange: boolean = !this.d3eChanges.contains(AllProducts._ERRORS);
  ...
  public setCount(val: number): void {
    ...
    this.updateD3EChanges(AllProducts._COUNT, this._count);
  ...
  public setItems(val: Array<Product>): void {
    ...
    if (!this.d3eChanges.contains(AllProducts._ITEMS)) {
      let _old: Array<Product> = Array.from(this._items);

      this.updateD3EChanges(AllProducts._ITEMS, _old);
    }
  ...
  public addToItems(val: Product, index: number = -1): void {
    let _old: Array<Product> = [];

    let _isNewChange: boolean = !this.d3eChanges.contains(AllProducts._ITEMS);
  ...
  public removeFromItems(val: Product): void {
    let _old: Array<Product> = [];
  ...
  public set(field: number, value: any): void {
    switch (field) {
      case AllProducts._STATUS: {
        this.setStatus(ResultStatus.values[value as number]);
        break;
      }

      case AllProducts._ERRORS: {
        this.setErrors((value as Array<any>).cast<string>().toList());
        break;
      }

      case AllProducts._COUNT: {
        this.setCount(value as number);
        break;
      }

      case AllProducts._ITEMS: {
        this.setItems((value as Array<any>).cast<Product>().toList());
        break;
      }
    }
  }
