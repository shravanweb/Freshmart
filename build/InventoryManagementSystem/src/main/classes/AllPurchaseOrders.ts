...
export default class AllPurchaseOrders extends DBObject {
  ...
  private _items: Array<PurchaseOrder> = ListWrapper.vanilla(
    this,
    "items",
  ...
  public setStatus(val: ResultStatus): void {
    ...
    this.updateD3EChanges(AllPurchaseOrders._STATUS, this._status.index);
  ...
  public setErrors(val: Array<string>): void {
    ...
    if (!this.d3eChanges.contains(AllPurchaseOrders._ERRORS)) {
      let _old: Array<string> = Array.from(this._errors);

      this.updateD3EChanges(AllPurchaseOrders._ERRORS, _old);
    }
  ...
  public setCount(val: number): void {
    ...
    this.updateD3EChanges(AllPurchaseOrders._COUNT, this._count);

    this._count = val;

    this.fire("count", this);
  }
  public get items(): Array<PurchaseOrder> {
    return this._items;
  }
  public setItems(val: Array<PurchaseOrder>): void {
    ...
    if (!this.d3eChanges.contains(AllPurchaseOrders._ITEMS)) {
      let _old: Array<PurchaseOrder> = Array.from(this._items);

      this.updateD3EChanges(AllPurchaseOrders._ITEMS, _old);
    }
  ...
  public addToItems(val: PurchaseOrder, index: number = -1): void {
    let _old: Array<PurchaseOrder> = [];
  ...
  public removeFromItems(val: PurchaseOrder): void {
    let _old: Array<PurchaseOrder> = [];
  ...
  public set(field: number, value: any): void {
    switch (field) {
      case AllPurchaseOrders._STATUS: {
        this.setStatus(ResultStatus.values[value as number]);
        break;
      }

      case AllPurchaseOrders._ERRORS: {
        this.setErrors((value as Array<any>).cast<string>().toList());
        break;
      }

      case AllPurchaseOrders._COUNT: {
        this.setCount(value as number);
        break;
      }

      case AllPurchaseOrders._ITEMS: {
        this.setItems((value as Array<any>).cast<PurchaseOrder>().toList());
        break;
      }
    }
  }
  ...
  public equals(other: any): boolean {
    return (
      this === other ||
      (other instanceof AllPurchaseOrders &&
