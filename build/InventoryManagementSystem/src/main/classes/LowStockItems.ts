...
export default class LowStockItems extends DBObject {
  ...
  private _items: Array<WarehouseStock> = ListWrapper.vanilla(
    this,
    "items",
  ...
  public setStatus(val: ResultStatus): void {
    ...
    this.updateD3EChanges(LowStockItems._STATUS, this._status.index);
  ...
  public setErrors(val: Array<string>): void {
    ...
    if (!this.d3eChanges.contains(LowStockItems._ERRORS)) {
      let _old: Array<string> = Array.from(this._errors);

      this.updateD3EChanges(LowStockItems._ERRORS, _old);
    }
  ...
  public setCount(val: number): void {
    ...
    this.updateD3EChanges(LowStockItems._COUNT, this._count);
  ...
  public setItems(val: Array<WarehouseStock>): void {
    ...
    if (!this.d3eChanges.contains(LowStockItems._ITEMS)) {
      let _old: Array<WarehouseStock> = Array.from(this._items);

      this.updateD3EChanges(LowStockItems._ITEMS, _old);
    }
  ...
  public addToItems(val: WarehouseStock, index: number = -1): void {
    let _old: Array<WarehouseStock> = [];

    let _isNewChange: boolean = !this.d3eChanges.contains(LowStockItems._ITEMS);
  ...
  public removeFromItems(val: WarehouseStock): void {
    let _old: Array<WarehouseStock> = [];
  ...
  public set(field: number, value: any): void {
    switch (field) {
      case LowStockItems._STATUS: {
        this.setStatus(ResultStatus.values[value as number]);
        break;
      }

      case LowStockItems._ERRORS: {
        this.setErrors((value as Array<any>).cast<string>().toList());
        break;
      }

      case LowStockItems._COUNT: {
        this.setCount(value as number);
        break;
      }

      case LowStockItems._ITEMS: {
        this.setItems((value as Array<any>).cast<WarehouseStock>().toList());
        break;
      }
    }
  }
  ...
  public equals(other: any): boolean {
    return (
      this === other ||
      (other instanceof LowStockItems &&
