...
export default class ExpiringBatches extends DBObject {
  ...
  private _items: Array<StockBatch> = ListWrapper.vanilla(
    this,
    "items",
  ...
  public setStatus(val: ResultStatus): void {
    ...
    this.updateD3EChanges(ExpiringBatches._STATUS, this._status.index);
  ...
  public setErrors(val: Array<string>): void {
    ...
    if (!this.d3eChanges.contains(ExpiringBatches._ERRORS)) {
      let _old: Array<string> = Array.from(this._errors);

      this.updateD3EChanges(ExpiringBatches._ERRORS, _old);
    }
  ...
  public setCount(val: number): void {
    ...
    this.updateD3EChanges(ExpiringBatches._COUNT, this._count);

    this._count = val;

    this.fire("count", this);
  }
  public get items(): Array<StockBatch> {
    return this._items;
  }
  public setItems(val: Array<StockBatch>): void {
    ...
    if (!this.d3eChanges.contains(ExpiringBatches._ITEMS)) {
      let _old: Array<StockBatch> = Array.from(this._items);

      this.updateD3EChanges(ExpiringBatches._ITEMS, _old);
    }
  ...
  public addToItems(val: StockBatch, index: number = -1): void {
    let _old: Array<StockBatch> = [];
  ...
  public removeFromItems(val: StockBatch): void {
    let _old: Array<StockBatch> = [];
  ...
  public set(field: number, value: any): void {
    switch (field) {
      case ExpiringBatches._STATUS: {
        this.setStatus(ResultStatus.values[value as number]);
        break;
      }

      case ExpiringBatches._ERRORS: {
        this.setErrors((value as Array<any>).cast<string>().toList());
        break;
      }

      case ExpiringBatches._COUNT: {
        this.setCount(value as number);
        break;
      }

      case ExpiringBatches._ITEMS: {
        this.setItems((value as Array<any>).cast<StockBatch>().toList());
        break;
      }
    }
  }
  ...
  public equals(other: any): boolean {
    return (
      this === other ||
      (other instanceof ExpiringBatches &&
